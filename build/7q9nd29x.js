...
` : "") : (typeof t2 == "function" ? (this.clear().build(true), t2.call(this, this), this.build(false)) : this.plain(t2), this);
  ...
  var zi = function() {
    ...
    return s(t2, [{ key: "isValidDate", value: function(t3) {
      ...
      var i2 = this.w.globals.locale, a2 = this.w.config.xaxis.labels.datetimeUTC, s2 = ["\x00"].concat(f(i2.months)), r2 = ["\x01"].concat(f(i2.shortMonths)), n2 = ["\x02"].concat(f(i2.days)), o2 = ["\x03"].concat(f(i2.shortDays));
      function l2(t4, e3) {
        var i3 = t4 + "";
      ...
      if (Array.isArray(o2.x))
        for (var d2 = 0;d2 < o2.x.length; d2++) {
          ...
          if (r2.config.chart.type !== "line" && r2.config.chart.type !== "area" || r2.globals.comboCharts || r2.config.tooltip.intersect || (f2 += " no-pointer-events"), (Array.isArray(r2.config.markers.size) ? r2.globals.markers.size[e2] > 0 : r2.config.markers.size > 0) || s2 || c2) {
            ...
            r2.globals.pointsArray[e2] === undefined && (r2.globals.pointsArray[e2] = []), r2.globals.pointsArray[e2].push([o2.x[d2], o2.y[d2]]);
        }
      return l2;
  ...
  var Ji = function() {
    ...
    return s(t2, [{ key: "scaleSvgNode", value: function(t3, e2) {
      ...
      return new Promise(function(e2) {
        ...
        s2 !== 1 && t3.scaleSvgNode(n2, s2), t3.convertImagesToBase64(n2).then(function() {
...
`);
  ...
  var ma = function(t2) {
    ...
    return s(a2, [{ key: "init", value: function(t3) {
      ...
      var { context: e3, zoomtype: i2 } = t3, a3 = this.w, s2 = e3, r2 = this.xyRatios, n2 = this.ctx.toolbar;
      ...
      if (a3.config.yaxis.forEach(function(t4, e4) {
        ...
        if (a3.globals.zoomEnabled) {
          ...
          if (a3.globals.zoomed = true, a3.config.xaxis.convertedCatToNumeric && (h2 = Math.floor(h2), c2 = Math.floor(c2), h2 < 1 && (h2 = 1, c2 = a3.globals.dataPoints), c2 - h2 < 2 && (c2 = h2 + 1)), i2 !== "xy" && i2 !== "x" || (p2 = { min: h2, max: c2 }), i2 !== "xy" && i2 !== "y" || g2.forEach(function(t4, e4) {
          ...
          var x2 = { xaxis: p2 };
          a3.config.chart.group || (x2.yaxis = g2), s2.ctx.updateHelpers._updateOptions(x2, false, s2.w.config.chart.animations.dynamicAnimation.enabled), typeof a3.config.chart.events.zoomed == "function" && n2.zoomCallback(p2, g2);
        } else if (a3.globals.selectionEnabled) {
          var b2, m2 = null;
          b2 = { min: h2, max: c2 }, i2 !== "xy" && i2 !== "y" || (m2 = v.clone(a3.config.yaxis)).forEach(function(t4, e4) {
            m2[e4].min = u2[e4], m2[e4].max = d2[e4];
          }), a3.globals.selection = s2.selection, typeof a3.config.chart.events.selection == "function" && a3.config.chart.events.selection(s2.ctx, { xaxis: b2, yaxis: m2 });
        }
      }
  ...
  var ya = function() {
    ...
    return s(t2, [{ key: "drawSeriesTexts", value: function(t3) {
      ...
      b2 && (b2.innerHTML = c2 !== undefined ? c2 : ""), f2[0] && f2[0].classList.contains("apexcharts-tooltip-marker") && (l2.config.tooltip.marker.fillColors && Array.isArray(l2.config.tooltip.marker.fillColors) && (o2 = l2.config.tooltip.marker.fillColors[e2]), f2[0].style.backgroundColor = o2), l2.config.tooltip.marker.show || (f2[0].style.display = "none");
      ...
      typeof l2 == "string" ? n2.innerHTML = l2 : (l2 instanceof Element || typeof l2.nodeName == "string") && (n2.innerHTML = "", n2.appendChild(l2));
    } }]), t2;
  }();
  var wa = function() {
    function t2(e2) {
  ...
  var La = function() {
    ...
    return s(t2, [{ key: "handleBarDataLabels", value: function(t3) {
      ...
      c2 = Math.abs(c2);
      var w2 = e2.config.plotOptions.bar.dataLabels.orientation === "vertical", k2 = this.barCtx.barHelpers.getZeroValueEncounters({ i: i2, j: a2 }).zeroEncounters;
      l2 -= b2 / 2;
  ...
  var Fa = function() {
    ...
    return s(t2, [{ key: "sameValueSeriesFix", value: function(t3, e2) {
      ...
      var { series: e2, realIndex: i2, x: a2, y: s2, i: r2, j: n2, prevY: o2 } = t3, l2 = this.w, h2 = [], c2 = [];
      if (n2 === 0) {
    ...
    return s(t2, [{ key: "draw", value: function(t3) {
      ...
      return t3.forEach(function(t4) {
        ...
        n3.forEach(function(r3, n4) {
          ...
          i2.config.plotOptions.treemap.dataLabels.format === "truncate" && (w2 = parseInt(i2.config.dataLabels.style.fontSize, 10), k2 = e2.truncateLabels(k2, w2, h3, c3, d2, u2));
          var A2 = null;
          i2.globals.series[o2][n4] && (A2 = e2.helpers.calculateDataLabels({ text: k2, x: (h3 + d2) / 2, y: (c3 + u2) / 2 + e2.strokeWidth / 2 + w2 / 3, i: o2, j: n4, colorProps: p2, fontSize: w2, series: t3 })), i2.config.dataLabels.enabled && A2 && e2.rotateToFitLabel(A2, w2, k2, h3, c3, d2, u2), l2.add(g2), A2 !== null && l2.add(A2);
        }), l2.add(c2), r2.add(l2);
      }), r2;
  ...
  var qa = function() {
    ...
    return s(t2, [{ key: "setupElements", value: function() {
      ...
      r2.series.forEach(function(e3, a3) {
        ...
        n2[s3] ? (s3 === "rangeArea" ? (n2[s3].series.push(r2.seriesRangeStart[a3]), n2[s3].seriesRangeEnd.push(r2.seriesRangeEnd[a3])) : n2[s3].series.push(e3), n2[s3].i.push(a3), s3 !== "column" && s3 !== "bar" || (i2.globals.columnSeries = n2.column)) : ["heatmap", "treemap", "pie", "donut", "polarArea", "radialBar", "radar"].includes(s3) ? l2 = s3 : s3 === "bar" ? (n2.column.series.push(e3), n2.column.i.push(a3)) : console.warn("You have specified an unrecognized series type (".concat(s3, ").")), o2 !== s3 && s3 !== "scatter" && h2++;
...
// octopus.ts
var apiCache = {};
var apiRoot = "https://api.octopus.energy/v1/";
var getPeriodTo = (periodFrom) => {
  const periodTo = new Date(periodFrom);
  periodTo.setDate(periodTo.getDate() + 1);
  return periodTo;
};
var getData = async (path, token) => {
  ...
  if (apiCache[cacheKey]) {
    return JSON.parse(apiCache[cacheKey]);
  }
...
var getAccountData = async (account, token) => await getData(`${apiRoot}accounts/${account}/`, token);
var getMeterConsumption = async (account, token, periodFrom) => {
  const periodTo = getPeriodTo(periodFrom);
  const accountData = await getAccountData(account, token);
  if (!accountData)
    return null;
  const meterPoints = accountData.properties.flatMap((property) => property.electricity_meter_points).filter((point) => !point.is_export);
  const meterConsumption = {};
  const meters = meterPoints.flatMap((point) => point.meters.map((meter) => ({ mpan: point.mpan, serialNumber: meter.serial_number })));
  await Promise.all(meters.map(async ({ mpan, serialNumber }) => {
    const meterData = await getData(`${apiRoot}electricity-meter-points/${mpan}/meters/${serialNumber}/consumption?period_from=${periodFrom.toISOString()}&period_to=${periodTo.toISOString()}`, token);
    if (meterData && meterData.results.length) {
      ...
      meterConsumption[serialNumber] = meterConsumption[serialNumber].sort(({ periodFrom: periodFrom1 }, { periodFrom: periodFrom2 }) => periodFrom1.getTime() - periodFrom2.getTime()).slice(0, 48);
    }
  }));
  return Object.keys(meterConsumption).length ? meterConsumption : null;
};
var getRates = async (region, periodFrom) => {
  const periodTo = getPeriodTo(periodFrom);
  const tariffs = await getData(`${apiRoot}products/`);
  const agileTariff = tariffs.results.find((tariff) => tariff.display_name === "Agile Octopus" && tariff.brand === "OCTOPUS_ENERGY");
  ...
  const regionUnitRatesHref = regionTariff.direct_debit_monthly.links.find((link) => link.rel === "standard_unit_rates")?.href + `?period_from=${periodFrom.toISOString()}&period_to=${periodTo.toISOString()}`;
  if (!regionUnitRatesHref) {
    return [];
  }
  const regionUnitRates = await getData(regionUnitRatesHref);
  return regionUnitRates.results.reverse().map(({ valid_from, value_inc_vat }) => ({
...
// charts.ts
var ratesChartInstance;
var costChartInstance;
var ratesChartElement;
var costChartElement;
var setElements = (ratesChart, costChart) => {
...
var roundToTwoDecimals = (num) => Math.round(num * 100) / 100;
var getConsumptionCosts = async (rates, meterConsumption) => {
  ...
  for (const serial_number in meterConsumption) {
    ...
    for (const consumption of meterConsumption[serial_number]) {
      const ratesForPeriod = rates.find((rate) => rate.periodFrom.getTime() == consumption.periodFrom.getTime())?.price || 0;
      consumptionCosts[serial_number].push([consumption.periodTo, consumption.value * ratesForPeriod]);
    }
    consumptionCosts[serial_number] = consumptionCosts[serial_number].sort(([periodFrom1, _], [periodFrom2, __]) => periodFrom1.getTime() - periodFrom2.getTime());
  }
  return Object.keys(consumptionCosts).length ? consumptionCosts : null;
};
var createRatesChartOptions = async (region, periodFrom, meterConsumption) => {
  ...
  let times = Object.keys(consumptionCosts).length ? Object.values(consumptionCosts)[0].map(([periodFrom2]) => periodFrom2.toISOString().slice(11, 16)) : rates.map(({ periodFrom: periodFrom2 }) => periodFrom2.toISOString().slice(11, 16));
...
var renderRatesChart = async (region, periodFrom, account, token) => {
  ...
  if (!ratesChartInstance) {
    ratesChartInstance = new import_apexcharts.default(ratesChartElement, chartOptions);
...
var createCostChartOptions = async (region, periodFrom, consumption) => {
  const rates = await getRates(region, periodFrom);
  ...
  for (const serial_number in consumptionCosts) {
    ...
    for (const serial_number2 in consumption) {
      for (const value of consumption[serial_number2]) {
  ...
  const times = rates.map(({ periodFrom: periodFrom2 }) => periodFrom2.toISOString().slice(11, 16));
  const options = {
    colors: ["rgb(250, 152, 255)", "rgb(16, 195, 149)", "rgb(88, 64, 255)"],
    series,
    chart: {
...
var renderCostChart = async (region, periodFrom, account, token) => {
  document.body.style.cursor = "wait";
  const consumption = account && token ? await getMeterConsumption(account, token, periodFrom) : null;
  if (!consumption) {
    costChartInstance && costChartInstance.destroy();
  ...
  const chartOptions = Object.keys(consumption).length ? await createCostChartOptions(region, periodFrom, consumption) : {};
  if (!costChartInstance) {
    costChartInstance = new import_apexcharts.default(costChartElement, chartOptions);
...
var render = async (region, periodFromValue, account, token) => {
  await Promise.all([
...
// index.ts
var getStorageValue = (key) => {
  let value = localStorage.getItem("datastar");
  if (!value)
    return;
  return JSON.parse(value)[key];
};
var getRegion = () => getStorageValue("region");
var getAccount = () => getStorageValue("account");
var getToken = () => getStorageValue("token");
var signOut = destroy;
var signIn = async (account, token) => {
  if (account && token) {
    document.body.style.cursor = "wait";
    const accountData = await getAccountData(account, token);
    document.body.style.cursor = "default";
...
var setNextDay = (periodFromValue, maxPeriodFromValue) => {
...
var setPreviousDay = (periodFromValue) => {
  const periodFrom = new Date(periodFromValue);
  periodFrom.setDate(periodFrom.getDate() - 1);
  return periodFrom.toISOString().slice(0, 10);
};
...
var renderCharts = (region, periodFromValue, account, token) => {
  render(region, periodFromValue, account, token);
};
var onload = (ratesChartElement2, costChartElement2, renderCharts2) => {
  setElements(ratesChartElement2, costChartElement2);
  renderCharts2();
  setTimeout(() => {
    renderCharts2();
    setInterval(() => renderCharts2(), 1800000);
  }, nextRateChange());
};
export {
  signOut,
